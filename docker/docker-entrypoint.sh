...
file_diybotset_sample="$dir_sample/diybotset.sample.json"
file_diybotset_user="$dir_config/diybotset.json"
...
if [ ! -s $file_diybotset_user ]; then
    echo -e "检测到 $file_diybotset_user 不存在，从示例文件复制一份用于初始化...\n"
    cp -fv $file_diybotset_sample $file_diybotset_user 
    echo
fi
